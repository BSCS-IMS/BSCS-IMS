...
export default function DeleteResellerModal({ onClose, reseller, onSuccess }) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await axios.delete(`/api/resellers/${reseller.id}`)
      ...
      const errorMsg = err.response?.data?.message || 'Failed to delete reseller'
      ...
      <div className="bg-white w-full max-w-sm max-h-[90vh] overflow-y-auto overflow-x-hidden rounded-xl shadow-xl relative">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 pt-5 pb-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-[#1F384C]">Delete Reseller</h2>
          </div>
        ...
        <div className="px-4 sm:px-6 pb-4">
          <div className="bg-[#fff8f8] border border-[#ffcdd2] rounded-lg p-3 mb-4">
            <p className="text-xs font-medium text-[#1F384C]">{reseller.businessName}</p>
            {reseller.contactNumber && (
              <p className="text-xs text-[#6b7280]">Contact: {reseller.contactNumber}</p>
            )}
          </div>

          <p className="text-xs text-[#374151] leading-relaxed">
            Are you sure you want to delete this reseller? This action will remove the reseller and all their assigned products.
          </p>
        </div>
